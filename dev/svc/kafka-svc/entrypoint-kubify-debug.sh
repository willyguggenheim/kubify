#!/bin/bash -e
# unset AWS_PROFILE
keytool -exportcert -alias CARoot -keystore /certs/kafka.server.keystore.jks -rfc -file root-ca.pem -storepass password
encoded_cert=$(cat ./root-ca.pem | base64)

# Put latest cert in localstack SSM

# TODO: Verify if this needs to occur for all stages, not just local
if [[ "$STAGE" == "local" ]]; then

    aws ssm get-parameter --name "/kubify/${STAGE}/secrets/svc/kafka-svc/secrets/root-ca" --endpoint-url http://localstack-aws-svc.demo.svc:4566 --no-verify-ssl || \
        aws ssm put-parameter --name "/kubify/${STAGE}/secrets/svc/kafka-svc/secrets/root-ca" --value "$encoded_cert" --type SecureString --endpoint-url http://localstack-aws-svc.demo.svc:4566 --no-verify-ssl
    aws ssm delete-parameter --name "/kubify/${STAGE}/secrets/svc/kafka-svc/secrets/root-ca" --endpoint-url http://localstack-aws-svc.demo.svc:4566 --no-verify-ssl
    aws ssm put-parameter --name "/kubify/${STAGE}/secrets/svc/kafka-svc/secrets/root-ca" --value "$encoded_cert" --type SecureString --endpoint-url http://localstack-aws-svc.demo.svc:4566 --no-verify-ssl
fi
...
exec "$KAFKA_HOME/bin/kafka-server-start.sh" "$KAFKA_HOME/config/server.properties" || echo "kafka start failed. restarting ... " && sleep 3 && exec "$KAFKA_HOME/bin/kafka-server-start.sh" "$KAFKA_HOME/config/server.properties"
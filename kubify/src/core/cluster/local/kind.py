...
def get_pod_object(namespace, pod_name,):
    pod = pykube.Pod.objects(api).filter(namespace=namespace).get(name=pod_name)
    pod.obj["spec"]["containers"][0]["image"]
    
def deploy_obj(api, obj): 
    pykube.Deployment(api, obj).create()

def delete_obj(api, obj):
...
def create_namespaced_deployment(deployment_name, deployment_manifest, namspace="default"):
    ...
    while True:
        try:
            response = v1.read_namespaced_deployment_status(name=deployment_name, namespace=namspace)
